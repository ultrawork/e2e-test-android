...
class RetrofitViewModelE2ETests {
    ...
    fun setUp() {
        ...
        // Use instrumentation args or fallback to emulator host address
    ...
    /**
     * Helper: obtain a dev token from the backend.
     */
     ...
     * Verifies that backend returns a valid JWT dev-token with status 200.
     */
    @Test
     ...
     * Verifies that an authorized request returns 200 and a JSON array.
     */
    @Test
     ...
     * Verifies that a request without Authorization header is rejected.
     */
    @Test
     ...
     * Verifies that an authorized user can create a note and receives the created entity.
     */
    @Test
    fun sc004_postNoteReturns201WithId() {
        ...
        val noteJson = gson.toJson(mapOf(
            "title" to "Test Note v27",
            "content" to "Test content for E2E verification"
        ))
        ...
        assertEquals("Test Note v27", json.get("title").asString)
        assertEquals("Test content for E2E verification", json.get("content").asString)
    }

    /**
     * SC-005: POST /api/notes without content — 400 Bad Request.
     * Verifies that backend validates the required 'content' field.
     */
    @Test
    ...
    fun sc006_deleteExistingNoteReturns204() {
        ...
        val noteJson = gson.toJson(mapOf(
            "title" to "Note to delete",
            "content" to "This note will be deleted in E2E test"
        ))
     ...
     * Verifies that deleting a non-existent note returns 404.
     */
    @Test
