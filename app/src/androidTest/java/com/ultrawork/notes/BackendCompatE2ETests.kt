...
/**
 * E2E backend compatibility tests for Android v34.
 *
 * Verifies the backend API contract (Retrofit + AuthInterceptor + ViewModel)
 * against backend v34. Scenarios SC-001..SC-004 from android-notes-api.md.
 */
@RunWith(AndroidJUnit4::class)
class BackendCompatE2ETests {
     ...
     * Verifies backend v34 returns a valid JWT dev-token with status 200.
     */
    @Test
     ...
     * Verifies request without Authorization header is rejected by backend v34.
     */
    @Test
     ...
     * Verifies authorized user can create a note with title and content via backend v34.
     */
    @Test
    fun sc004_postNoteReturns201WithId() {
        ...
        val noteJson = gson.toJson(mapOf(
            "title" to "Test Note v34",
            "content" to "Test content for E2E v34 backend compatibility"
        ))
        ...
        assertEquals("Test Note v34", json.get("title").asString)
        assertEquals("Test content for E2E v34 backend compatibility", json.get("content").asString)
        ...
        val noteId = json.get("id").asString
